...
public class RegisterDeviceTests : IDisposable
{
    ...
    [Theory]
    [InlineData("test-ABC-123-XYZ-001", "secret-ABC-123-XYZ-001", "")]
    [InlineData("test-ABC-123-XYZ-002", "secret-ABC-123-XYZ-002", null)]
    public async Task ReturnErrorWhenMissedFirmware(string serialNumber, string secret, string firmware)
    {
        _client.DefaultRequestHeaders.Add("x-device-shared-secret", secret);
        var response = await _client
            .PostAsync($"/api/v1/device/{serialNumber}/register?firmwareVersion={firmware}", default);
        ...
        var problemDetails = await response.Content.ReadFromJsonAsync<ValidationProblemDetails>();
        string expectedError = "The firmwareVersion field is required.";
        Assert.Equal(HttpStatusCode.BadRequest, response.StatusCode);
        Assert.Equal(expectedError, problemDetails?.Errors["firmwareVersion"].FirstOrDefault());
    }
